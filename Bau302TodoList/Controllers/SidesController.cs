...
namespace Bau302TodoList.Controllers
{
    ...
    public class SidesController : Controller
    {
        ...
        public async Task<ActionResult> Create([Bind(Include = "Id,Name,CreateDate,CreateBy,UpdateDate,UpdateBy")] Side side)
        {
            if (ModelState.IsValid)
            {
                side.CreateDate = DateTime.Now;
                side.CreateBy = "Unknow";
                side.UpdateDate = DateTime.Now;
                side.UpdateBy = "Unknow";
                db.Sides.Add(side);
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> Edit([Bind(Include = "Id,Name,CreateDate,CreateBy,UpdateDate,UpdateBy")] Side side)
        {
            if (ModelState.IsValid)
            {
                side.UpdateDate = DateTime.Now;
